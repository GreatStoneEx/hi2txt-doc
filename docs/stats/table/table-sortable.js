...
var defaultConfig = {
    ...
    columns: [],
    sorting: true,
	sortingFunction: function(obj, sortKey) { return obj[sortKey]; },
    ...
    _createClass(TableSortable, [{
                ...
                forIn(columns, function (val, key) {
                    ...
                    //if (row[key] !== undefined) {
                        ...
                        if (isFunction(processHtml)) {
                            ...
                            tbd.html((row[key]===undefined)?"":row[key]); // FIX PM
                        }
                        tbr.append(tbd);
                    //}
                });
        ...
        value: function sortData(sortKey) {
            var _config6 = this.config,
				...
				sortingFunction = _config6.sortingFunction;
                ...
                if (this.sortDirection === sortDirection.ASC) {
                    this.config.data = data.sort(function (a, b) {
						...
						if (sortingFunction != null) {
						  if (sortingFunction.call(this, a, sortKey) < sortingFunction.call(this, b, sortKey)) return -1;
                          if (sortingFunction.call(this, a, sortKey) > sortingFunction.call(this, b, sortKey)) return 1;
						} else {
						  if (a[sortKey] < b[sortKey]) return -1;
                          if (a[sortKey] > b[sortKey]) return 1;	
						}
						//if (transformForSortDefault(a[sortKey], sortKey) < transformForSortDefault(b[sortKey], sortKey)) return -1;
                        //if (transformForSortDefault(a[sortKey], sortKey) > transformForSortDefault(b[sortKey], sortKey)) return 1;
						//if (a[sortKey] < b[sortKey]) return -1;
                        //if (a[sortKey] > b[sortKey]) return 1;
                        return 0;
